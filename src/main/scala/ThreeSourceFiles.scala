...
object ThreeSourceFiles {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val CityList= spark.read.format("csv")
      .option("mode", "FAILFAST")
      .option("header","true")
      .option("inferSchema", "true")
      .load("Source/CityList.csv")
    ...
    //Loading the CSV File
    val CityListA= spark.read.format("json")
      .option("mode", "FAILFAST")
      .option("header","true")
      .option("inferSchema", "true")
      .load("Source/CityListA.json")
    ...
    //Loading the CSV File
    val CityListB= spark.read.format("avro")
      .option("mode", "FAILFAST")
      .option("header","true")
      .option("inferSchema", "true")
      .load("Source/CityListB.avro")
    ...
    val result4=spark.sql("select count(*) from CityListCombined")
    result4.show(100)
    print("What is the city with the largest population?")
    val result5=spark.sql("select max(Population) from CityListCombined ")
    result5.show(100)
